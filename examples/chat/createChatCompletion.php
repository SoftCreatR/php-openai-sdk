...
try {
    $response = $openAI->createChatCompletion($options);

    // Check if the response has a 200 status code (success)
    if ($response->getStatusCode() === 200) {
        // Decode the response body
        $chatCompletion = \json_decode($response->getBody(), true, 512, \JSON_THROW_ON_ERROR);

        // Print the chat completion information as a JSON string
        echo "============\n| Response |\n============\n\n";
        echo \json_encode($chatCompletion, \JSON_THROW_ON_ERROR | \JSON_PRETTY_PRINT);
        echo "\n\n============\n";
    } else {
